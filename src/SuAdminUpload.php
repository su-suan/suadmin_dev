...
use RedisException;
use SuAdmin\Event\UploadAfter;
use SuAdmin\Exception\NormalStatusException;
use SuAdmin\Utils\Str;
use SuAdmin\Interfaces\ServiceInterface\ConfigServiceInterface;
...
class SuAdminUpload
{
    ...
    protected SuAdminRequest $SuAdminRequest;
    ...
    public function __construct(
        ...
        SuAdminRequest           $SuAdminRequest,
        IdGeneratorInterface     $idGenerator
    ) {
        $this->factory = $factory;
        $this->evDispatcher = $evDispatcher;
        $this->configService = $configService;
        $this->SuAdminRequest = $SuAdminRequest;
    ...
    public function handleChunkUpload(array $data): array
    {
        ...
        $fs = su_admin_container()->get(\Hyperf\Support\Filesystem\Filesystem::class);
        ...
        if ($data['index'] === $data['total']) {
            ...
            $fileInfo = [
                ...
                'suffix' => $data['ext'],
                'size_byte' => $data['size'],
                'size_info' => su_admin_format_size((int) $data['size'] * 1024),
    ...
    public function handleSaveNetworkImage(array $data): array
    {
        ...
        try {
            ...
            if (! $hash) {
                throw new Exception(su_admin_translator('network_image_save_fail'));
            }
        ...
        $fileInfo = [
            ...
            'hash' => $hash,
            'size_byte' => $size,
            'size_info' => su_admin_format_size($size * 1024),
    ...
    protected function handleUpload(UploadedFile $uploadedFile, array $config): array
    {
        ...
        $fileInfo = [
            ...
            'size_info' => su_admin_format_size($uploadedFile->getSize() * 1024),
    ...
    {
        return $this->SuAdminRequest->getScheme();
    }
}
