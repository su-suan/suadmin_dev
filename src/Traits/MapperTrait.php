...
trait MapperTrait
{
    ...
    {
        if (! config('SuAdmin.remote_api_enabled')) {
            throw new SuAdminException('系统未启用【远程通用列表查询】', 500);
        }
        /* @var $model SuAdminModel */
        $model = $this->getModel();
        $query = null;
        ...
        return method_exists($this, 'handleItems')
            ? (new SuAdminCollection($this->handleItems($query->get($params['select'] ?? ['*']), $params)))->toArray()
    ...
    public function getTreeList(
        ...
        $params['_SuAdmin_tree'] = true;
        $params['_SuAdmin_tree_pid'] = $parentField;
    ...
    {
        // 对树型数据强行加个排序
        if (isset($params['_SuAdmin_tree'])) {
            $query->orderBy($params['_SuAdmin_tree_pid']);
        }
     ...
     * @return mixed
     */
    public function first(array $condition, array $column = ['*']): ?SuAdminModel
    {
    ...
    public function getModel(): SuAdminModel
    {
    ...
    public function import(string $dto, ?\Closure $closure = null): bool
    {
        return Db::transaction(function () use ($dto, $closure) {
            return (new SuAdminCollection())->import($dto, $this->getModel(), $closure);
        });
    }
     ...
     */
    public function dynamicRelations(SuAdminModel $model, &$config): void
    {
