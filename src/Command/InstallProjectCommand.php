 ...
 */
#[Command]
class InstallProjectCommand extends SuAdminCommand
{
     ...
     * 安装命令.
     */
    protected ?string $name = 'suadmin:install';
    ...
    {
        parent::configure();
        $this->setHelp('run "php bin/hyperf.php suadmin:install" install SuAdmin system');
        $this->setDescription('SuAdmin system install command');

        $this->addOption('option', '-o', InputOption::VALUE_OPTIONAL, 'input "-o reset" is re install SuAdmin');
    }
    ...
    {
        $this->line('-----------------------------------------------------------', 'comment');
        $this->line('Hello, welcome use SuAdmin system.', 'comment');
        ...
        if ($dbAnswer) {
            ...
            switch ($dbtype) {
                case 'mysql':
                    ...
                    $dbname = $this->ask('please input database name, default:', 'SuAdmin');
    ...
    {
        $this->line(PHP_EOL . ' SuAdmin set others items...' . PHP_EOL, 'comment');
        $this->call('suadmin:update');
        $this->call('suadmin:jwt-gen', ['--jwtSecret' => 'JWT_SECRET']);
        $this->call('suadmin:jwt-gen', ['--jwtSecret' => 'JWT_API_SECRET']);

        if (! file_exists(BASE_PATH . '/config/autoload/SuAdmin.php')) {
            $this->call('vendor:publish', ['package' => 'susuan/SuAdmin']);
        }
        ...
        if ($downloadFrontCode) {
            ...
            if (\shell_exec('which git')) {
                \system('git clone https://gitee.com/SuAdmin/SuAdmin-vue.git ./web/');
            } else {
...
default password: admin123', $this->getInfo(), SuAdmin::getVersion()), 'comment');
    }
