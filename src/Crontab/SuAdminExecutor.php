...
use SuAdmin\Crontab\Mutex\RedisServerMutex;
use SuAdmin\Crontab\Mutex\RedisTaskMutex;
use SuAdmin\Interfaces\Crontab\ServerMutex;
use SuAdmin\Interfaces\Crontab\TaskMutex;
use SuAdmin\Interfaces\ServiceInterface\CrontabLogServiceInterface;
...
class SuAdminExecutor
{
     ...
     * @throws NotFoundExceptionInterface
     */
    public function execute(SuAdminCrontab $crontab, bool $run = false): ?bool
    {
        if ((! $crontab instanceof SuAdminCrontab || ! $crontab->getExecuteTime()) && ! $run) {
            return null;
        }
        $diff = 0;
    ...
    protected function runInSingleton(SuAdminCrontab $crontab, \Closure $runnable): \Closure
    {
    ...
    protected function runOnOneServer(SuAdminCrontab $crontab, \Closure $runnable): \Closure
    {
        return function () use ($crontab, $runnable) {
    ...
    protected function decorateRunnable(SuAdminCrontab $crontab, \Closure $runnable): \Closure
    {
    ...
    protected function logResult(SuAdminCrontab $crontab, bool $isSuccess, $result = '')
    {
        if ($this->logger) {
            if ($isSuccess) {
        ...
        $data = [
            ...
            'status' => $isSuccess ? SuAdminModel::ENABLE : SuAdminModel::DISABLE,
